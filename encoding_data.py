...
def save_pikele(path, obj):
    with open(path, 'wb') as file:
        pickle.dump(obj, file)

def read_vc(vc, func_to_call, break_print='Error!', show=False, win_name='',break_key='q', **kwargs):
    while vc.isOpened():
        ret, frame = vc.read()
        if not ret:
            print(break_print)
...
def encoding_pickle(people_dir, encodings_path):
    encoding_dict = dict()
    face_detector = MTCNN()
...
if __name__ == '__main__':
    encoding_pickle(people_dir, encodings_path)